...
2. Chạy bằng Bash, chọn theo hệ điều hành:
   ...
   macOS:
   ```
   pbpaste | .venv/bin/zalo-bot-mcp-admin set-token
   ```

   Linux (xclip, không có thì wl-paste):
   ```
   xclip -o -selection clipboard | .venv/bin/zalo-bot-mcp-admin set-token
   ```
   ```
   wl-paste | .venv/bin/zalo-bot-mcp-admin set-token
   ```
